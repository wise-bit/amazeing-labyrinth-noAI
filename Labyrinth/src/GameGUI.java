        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (e.getSource() == Main.board[i][j]) {
                    ...
                    // System.out.println();
                    if (Main.deck.players.get(currentPlayer).movePlayer(i,j, Main.deck.players.get(currentPlayer).getRows(), Main.deck.players.get(currentPlayer).getColumns(), 1) == true){
                        System.out.printf("Changing position of player: %d\n", currentPlayer);
